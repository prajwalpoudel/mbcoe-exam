<div class="kt-portlet__body">
    <div class="form-group row validated">
        <div class="col-lg-6">
            {{ html()->label('Faculty') }}
            {{ html()->select('faculty_id', $faculties, $result->student->faculty_id ?? null)->class($errors->has('faculty_id') ? 'form-control is-invalid' : 'form-control')->id('faculty')  }}
            @error('faculty_id')
        ...
        <div class="col-lg-6">
            {{ html()->label('Student') }}
            {{ html()->select('student_id', [], null)->class($errors->has('student_id') ? 'form-control is-invalid' : 'form-control')->id('student')  }}
            @error('student_id')
            <div id="" class="error invalid-feedback"> {{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="kt-form__actions">
        <div class="row">
            ...
            <div class="col-lg-6">
                ...
                <a href="{{ route('admin.result.index') }}"  type="button" class="btn btn-secondary ">Cancel</a>
            </div>
        </div>
    </div>
</div>
